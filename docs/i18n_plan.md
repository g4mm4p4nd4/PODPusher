...
## Progress
- Completed: social generator now localizes loading and error states in both `en` and `es`, with component test coverage.

## Action Items
1. Audit `client/` for hard-coded strings. Track findings in a localisation backlog.
2. Generate translation extraction script (for example, `scripts/i18n_extract.ts`) that outputs new keys for translators.
